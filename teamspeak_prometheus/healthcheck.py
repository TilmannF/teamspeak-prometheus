"""Container healthcheck for teamspeak-prometheus. Run as ``healthcheck.py``.
...
from teamspeak_prometheus.config import resolve_config
from teamspeak_prometheus.errors import ExporterError
from teamspeak_prometheus.redaction import printable, secrets_for_redaction
...
def metrics_port(argv: list[str], env: Mapping[str, str]) -> int:
    ...
    try:
        return resolve_config(args, env).metrics_port
...
def exporter_secrets(env: Mapping[str, str], proc: Path = PROC) -> list[str]:
    ...
    return secrets_for_redaction(
        password_candidates(args, exporter_env)
        + password_candidates(argparse.Namespace(), env)
    )
...
def main(
    ...
    try:
        ...
        print(printable(f'unhealthy: {err}', secrets))
        return 1
    print(printable(f'healthy: metrics endpoint on port {port} answers', secrets))
    return 0
